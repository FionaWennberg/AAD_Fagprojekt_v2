...
"""
Train a backward Eelbrain decoder on preprocessed AAD envelope/EEG data.

Input
-----
A .npz file created by preprocess_backward_eelbrain.py.

Main keys:
    stim_att : attended envelope trials, each (time,)
    stim_ign : ignored envelope trials, each (time,)
    resp_tt  : EEG trials, each (time, channels)

Optional:
    stim_st, resp_st : single-talker sanity-check data
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backward Eelbrain decoding from preprocessed .npz data")

    parser.add_argument("--input", type=Path, required=True, help="Path to preprocessed .npz file")
    parser.add_argument("--outdir", type=Path, required=True, help="Directory for result files")

    parser.add_argument("--tstart", type=float, default=-0.5, help="Decoder lag-window start in seconds")
    parser.add_argument("--tstop", type=float, default=0.0, help="Decoder lag-window stop in seconds")
    parser.add_argument("--basis", type=float, default=0.05, help="Basis window length in seconds")
    parser.add_argument("--basis-window", default="hamming", help="Basis window type for eelbrain.boosting")
    parser.add_argument("--partitions", type=int, default=None, help="CV partitions; default is n_trials")
    parser.add_argument("--error", choices=["l1", "l2"], default="l2")
    parser.add_argument("--selective-stopping", type=int, default=1)
    parser.add_argument("--no-scale-data", action="store_true", help="Disable Eelbrain input normalization")
    ...
    parser.add_argument(
        "--save-resp",
        action="store_true",
        help="Also save resp_tt in prediction NPZ. Usually unnecessary because it is already in input NPZ.",
    )
    parser.add_argument(
        "--timing",
        action="store_true",
...
def main() -> None:
    ...
    # Update JSON once more so final save/total timing is included.
