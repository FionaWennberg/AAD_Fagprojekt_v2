...
using WAV stimuli from ``stimuli_audio`` and one final auditory-inspired envelope
predictor per stimulus.

This version keeps Pipeline 1 simple:
- backward AAD / envelope reconstruction
- one final envelope per target/masker
- no multiband model fitting yet

But instead of a plain broadband Hilbert envelope, the stimulus feature is now:

    audio
    -> ERB-spaced auditory-inspired filterbank
    -> Hilbert envelope in each band
    -> compress each band
    -> low-pass each band
...
This gives a stronger "single-envelope baseline" while still keeping the
TRF/AAD pipeline conceptually simple.

Performance improvements relative to the old version:
- cache stimulus envelopes so repeated WAV files are processed only once
- read event files only once per run
- optionally resample audio to a faster working rate before filterbank analysis
- use SOS filters for stability and speed
...
# -----------------------------------------------------------------------------
# Auditory-inspired envelope helpers
# -----------------------------------------------------------------------------
...
class EnvelopeConfig:
    # Final output rate/shape
    fs_out: float = 64.0

    # Auditory-inspired filterbank
    n_bands: int = 16
    fmin_hz: float = 150.0
    fmax_hz: float = 8000.0
    filter_order: int = 2

    # Optional faster working sample rate for audio front-end
    # 16 kHz is much faster than 48 kHz and still enough for fmax=8 kHz.
    audio_work_fs: float = 16000.0

    # Per-band envelope processing
    band_env_lp: float = 30.0
    compression: float = 0.3

    # Final single-envelope shaping
    hp_out: float = 1.0
    lp_out: float = 9.0
    final_order: int = 4
...
def _prepare_audio_workrate(x: np.ndarray, fs: float, cfg: EnvelopeConfig) -> tuple[np.ndarray, float]:
    """
    Optionally resample audio to a lower working fs for faster filterbank/envelope
    computation. This is one of the biggest preprocessing speedups.
    """
    ...
    # Ensure target fs can represent the chosen max band frequency.
    if cfg.fmax_hz >= 0.95 * nyq_target:
...
def extract_auditory_envelope(x: np.ndarray, fs: float, cfg: EnvelopeConfig) -> np.ndarray:
    """
    Extract one final auditory-inspired envelope by:
    - ERB-spaced bandpass filterbank
    ...
    This is not an exact gammatone implementation, but an ERB-spaced
    auditory-inspired approximation suitable for a stronger single-envelope
    baseline than a raw broadband Hilbert envelope.
    """
    ...
    if not band_envs:
        raise RuntimeError("No valid filterbank bands were created for auditory envelope extraction")

    envs = np.stack(band_envs, axis=1)  # (time, bands)
...
def preprocess_eeg_bdf(bdf_path: Path, cfg: EEGConfig) -> tuple[mne.io.BaseRaw, dict[str, Any]]:
    """
    Main EEG cost drivers here:
    - reading the BDF
    - filtering
    - resampling

    These are expected to take noticeable time, but they happen only once per run.
    """
...
def process_subject_backward(
    ...
    # Major speed improvement: cache processed envelopes by file path
    ...
    meta = {
        ...
        "notes": [
            ...
            "Single final envelope per stimulus",
            "Envelope is auditory-inspired: ERB-spaced filterbank -> Hilbert envelopes -> compression -> average",
            ...
            "Caching avoids recomputing the same WAV-derived envelope many times",
        ],
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preprocess Fuglsang WAV stimuli into backward-Eelbrain-ready auditory-envelope trial lists"
    )
    ...
    # Optional tuning parameters for experiments / HPC sweeps
